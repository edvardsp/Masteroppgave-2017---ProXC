...
First of all, ProXC++ is a concurrency library for C++. Only recently as of C++11 has the C++ programming language added concurrency primitives, such as threading and futures, to the standard library. However, C++ lacks any standard support for concurrency in user space. 
...
Before discussing the design of the runtime system, lets first identify what the responsibilities of the runtime system are. A ProXC++ program contains a set of processes, where each process represents some point of execution. The runtime system must be able to spawn, schedule, and synchronize these processes, i.e. process management. A runtime scheduler could therefore be responsible for process management. 
...
FIXME only on running at a time. cooperative scehduling.
...
Ready work processes are stored in a process queue called the \textit{ready queue}. The scheduler uses the ready queue to determine whether it has ready work processes or not, i.e. an idle scheduler has an empty ready queue. When a scheduler has a non\hyp{}empty ready queue, a work process is removed from the ready queue and resumed execution. Scheduling and rescheduling a work process is the same as adding the process to the ready queue.

A work process with a corresponding parent scheduler may spawn new work processes. These new processes are scheduled by the parent scheduler.

Work processes may suspend, or sleep, until a given or derived time point. These processes are added to a \textit{sleep queue}. The scheduler will frequently examine the sleep queue and reschedule any work processes with an expired time point.
