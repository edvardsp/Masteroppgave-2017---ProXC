...
func mandelbrot(wg *sync.WaitGroup, dim uint, lines_ch <-chan uint, data_ch chan<- MandelbrotData) {
	integral_x := (XMAX - XMIN) / float64(dim);
	integral_y := (YMAX - YMIN) / float64(dim);
	for line := range lines_ch {
...
func producer(wg *sync.WaitGroup, dim uint, lines_ch chan<- uint) {
...
func mandelbrot_program(dim uint) {
	...
	for round := 0; round < ROUNDS; round++ {
		lines_ch := make(chan uint)
		data_ch := make(chan MandelbrotData)
		start := time.Now()
		var wg sync.WaitGroup
		wg.Add( 2 + NUM_WORKERS )
		go producer(&wg, dim, lines_ch)
		go consumer(&wg, dim, data_ch)
		for worker := 0; worker < NUM_WORKERS; worker++ {
			go mandelbrot(&wg, dim, lines_ch, data_ch)
		}
		wg.Wait()
